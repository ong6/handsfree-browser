  ...
  */
  ...
  class Handsfree {
		...
		update(config, callback) {
			...
			["hands", "facemesh", "pose", "holistic", "handpose", "weboji"].forEach(
				(model) => {
					let wasEnabled = this.model[model].enabled;
					this.config[model] = this.model[model].config = merge_1(
						{},
						this.model[model].config,
						config[model]
					);

					if (wasEnabled && !this.config[model].enabled)
						this.model[model].disable();
					else if (!wasEnabled && this.config[model].enabled)
						this.model[model].enable(false);
				}
			);
  ...
  /////////////////////////////////////////////////////////////// */
		...
		start(callback) {
			...
			Object.keys(this.config.plugin).forEach((plugin) => {
				if (
					typeof this.config.plugin?.[plugin]?.enabled === "boolean" &&
					this.config.plugin[plugin].enabled
				) {
					this.plugin[plugin].enable();
				}
			});
		...
		loop() {
			...
			this.taggedPlugins.untagged?.forEach((pluginName) => {
				this.plugin[pluginName].enabled &&
					this.plugin[pluginName]?.onFrame(this.data);
			});
			...
			if (this.isDebugging) {
				...
				if (isUsingCamera) {
					this.debug.context.video.drawImage(
						this.debug.$video,
						0,
						0,
						this.debug.$canvas.video.width,
						this.debug.$canvas.video.height
					);
				}
			}

			this.isLooping &&
				requestAnimationFrame(() => this.isLooping && this.loop());
		}
		...
		setupDebugger() {
			...
			[
				...
				$canvas.classList.add(
					"handsfree-canvas",
					`handsfree-canvas-${model}`,
					`handsfree-hide-when-started-without-${model}`
				);
				...
				this.debug.$canvas[model].height = this.config.setup.canvas[
					model
				].height;
				...
				if (["weboji", "handpose"].includes(model)) {
					...
					this.debug.context[model] = this.debug.$canvas[model].getContext(
						"2d"
					);
				}
			});
